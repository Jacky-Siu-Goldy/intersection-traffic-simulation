...
public class HeadedDownOrigCar extends IntersectionSimCar implements LaneManagementInterface {
	...
	enum TheRightSpotToStartBlinking {
		GREATERTHANXSPOTSELECTED, LESSERTHANXSPOTSELECTED, GREATERTHANYSPOTSELECTED, LESSERTHANYSPOTSELECTED,
		DOESNTNEEDTOBLINK
	}
	...
	/**
	 * Important for the laneChangePoint Selected calculation needed for when the
	 * Life or right Blinkers will start blinking
	 */
	public void setTheRightSpotToStartBlinking() {
		...
		default:
			...
			System.out.print(
					"setTheRightSpotToStartBlinking(): ---> the blinkers toggle are set if everything is done right\n"
	...
	/**
	 * only need this to signal for Left Lane Change when necessary STILL NEED TO DO
	 * CONDITION FOR WHEN TO TURN BLINKERS ON FOR RIGHT TURN
	 */
	...
	/**
	 * only need this to signal for Left Lane Change when necessary STILL NEED TO DO
	 * CONDITION FOR WHEN TO TURN BLINKERS ON FOR LEFT TURN
	 */
	...
	// related stuff: check sectioned off stuff in code further down the bottom such
	// as
	/*
	 * 1) Left or right blinkers signal turn on will borrow from setTargetEnum()
	 * Helpers functions to determine the necessity to consider turning the blinkers
	 * on (section)
	 * 
	...
	// ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
	// Below: Used to determine the Car's body bounds(for keeping the car spaced
	// out)
	// -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
	...
	// ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
	protected static final double LEFTTURNEXITANGLE = 180; // aligns with the rotationAngle which means the
															// rotationAngle will be 180 degree when it exit the Left
															// turn circle (a circular path)

	protected static final double RIGHTTURNEXITANGLE = 0; // aligns with the rotationAngle which means the rotationAngle
															// will be 0 degree when it exit the Right turn circle (a
															// circular path)
	...
	/**
	 * This constant is used for the righTurn(double exitAngle, double radius)
	 * function if you keep calling that function without doing anything for what
	 * happens when it hit that exit angle your car will basically keep going in a
	 * circle, but as it goes around in a circle it will always hit that point which
	 * matches the exit angle, the rotationAngle will always align to match the exit
	 * angle at that instance And as the name suggest this constant is used for
	 * right lane change after the car makes a left turn at the intersection. When
	 * the car's rotationAngle align with this exitAngle, phase1 of the lane change
	 * process should end and go on to phase2
	 */
	public static final double R_ALTRIGHT_LANECHANGEEXITANGLE = 192;// right turn exit angle for right Lane change after
																	// left turn
	/**
	 * This constant is used for the leftTurn(double exitAngle, double radius)
	 * function if you keep calling that function without doing any for what happens
	 * when it hit that exit angle your car will basically keep going in a circle,
	 * but as it goes around in a circle it will always hit that point which matches
	 * the exit angle, the rotationAngle will always align to match the exit angle
	 * at that instance And as the name suggest this constant is used for right lane
	 * change after the car makes a left turn at the intersection. When the car's
	 * rotationAngle align with this exitAngle, phase3 of the lane change process
	 * should end and you do something else for example make it go straight
	 */
	public static final double L_ALTRIGHT_LANECHANGEEXITANGLE = 180;// left turn exit angle for right lane change after
																	// left turn

	/**
	 * see explanation for other LANCHANGEEXITANGLE
	 */
	public static final double L_ALTLEFT_LANECHANGEEXITANGLE = 135;// left turn exit angle for left lane change after
																	// left turn
	public static final double R_ALTLEFT_LANECHANGEEXITANGLE = 180;// right turn exit angle for left lane change after
																	// left turn

	/**
	 * Coordinate in the Y axis of when the car should start making a right turn to
	 * successfully make a right turn
	 */
	public static final double HEADINGDOWNRIGHTTURNY = 215; // Coordinate in the Y axis of when the car should start
															// making a right turn to successfully make a right turn

	/**
	 * Coordinate in the X axis of when the car should start making a left turn to
	 * successfully make a Left turn
	 */
	...
	public void setR_AltCancelRight_LaneChangeExitAngle() {
		...
		// System.out.println("r_ALTCANCELRIGHT_LANECHANGEEXITANGLE :" +
		// this.getR_AltCancelRight_LaneChangeExitAngle());
	}
	...
	public void SetR_BiCancelRight_LaneChangeExitAngle() {
		...
		// System.out.println("r_BICancelRight_LaneChangeExitAngle :" +
		// this.getR_BiCancelRight_LaneChangeExitAngle());
	}

	public void initializeBlinkersSetsAll4() {
		blinkersSet1 = new ImageView[] { front_R_BlinkerImageView_P, front_L_BlinkerImageView_P,
				rear_R_BlinkerImageView_P, rear_L_BlinkerImageView_P };

		blinkersSet2 = new ImageView[] { front_R_BlinkerImageView2_P, front_L_BlinkerImageView2_P,
				rear_R_BlinkerImageView2_P, rear_L_BlinkerImageView2_P };

		blinkersSet3 = new ImageView[] { front_R_BlinkerImageView3_P, front_L_BlinkerImageView3_P,
				rear_R_BlinkerImageView3_P, rear_L_BlinkerImageView3_P };

	}

	public void initializeRightBlinkersSets() {
		rightBlinkersSet = new ImageView[] { front_R_BlinkerImageView_P, rear_R_BlinkerImageView_P };

		rightBlinkersSet2 = new ImageView[] { front_R_BlinkerImageView2_P, rear_R_BlinkerImageView2_P };

		rightBlinkersSet3 = new ImageView[] { front_R_BlinkerImageView3_P, rear_R_BlinkerImageView3_P };

	}

	public void initializeLeftBlinkersSets() {
		leftBlinkersSet = new ImageView[] { front_L_BlinkerImageView_P, rear_L_BlinkerImageView_P };

		leftBlinkersSet2 = new ImageView[] { front_L_BlinkerImageView2_P, rear_L_BlinkerImageView2_P };

		leftBlinkersSet3 = new ImageView[] { front_L_BlinkerImageView3_P, rear_L_BlinkerImageView3_P };
	...
	/**
	 * Calculate the length of the Car from the front of the car to the back of the
	 * car should be called in the constructor to set the boundary of the car needed
	 * for use in spacing can be called again to calculate as the car faces in the
	 * direction of a new cardinal angle to account for screen warping of the
	 * dimensions
	 */
	public void setCustomCarLengthBaseOnCarSkin() {
		...
		case OnWhichLane.HEADING_LEFT_ON_RIGHT_LANE_LIST:
			temp_customCarLengthBaseOnCarSkin = Math
					.abs(this.getFrontLeftCornerPositionX() - this.getRearLeftCornerPositionX());
		...
		case OnWhichLane.HEADING_UP_ON_RIGHT_LANE_LIST:
			temp_customCarLengthBaseOnCarSkin = Math
					.abs(this.getFrontLeftCornerPositionY() - this.getRearLeftCornerPositionY());
			setCustomCarLengthBaseOnCarSkin(temp_customCarLengthBaseOnCarSkin);
			break;
		case OnWhichLane.HEADING_UP_ON_LEFT_LANE_LIST:
			temp_customCarLengthBaseOnCarSkin = Math
	...
	public void setTargetLaneEnum() {// this is class specific!!!!!
		// System.out.println("setTargetLaneEnum() ---> carIntention: " +
		// this.carIntention);
		Stream.of(MapUtility.entry(isBI_ONRLWANTLLC(), TargetLane.HEADING_DOWN_ON_LEFT_LANE_LIST),
				...
				// .peek(e -> System.out.println("Checking TargetLane: " + e.getValue() + " -> "
				// + e.getKey())) // 👈 Here
				.map(Map.Entry::getValue).findFirst().ifPresentOrElse(lane -> {
					this.setTargetLaneList(lane);
				}, () -> {
					/*
	...
	/**
	 * need to do a needToDoThisEveryTimeSetTargetFrontCarBlindSpotCarRearCar(){}
	 * --->method
	 */
	 ...
	 */
	public void setTargetLaneCarsDetection(ObservableList<IntersectionSimCar> theListOfTheLaneTheTargetedCarIsOn) {// Should
																													// be
																													// not
																													// Class
																													// Specific
		...
		if (theListOfTheLaneTheTargetedCarIsOn.size() > 0) {
			 ...
			 * setTargetLaneFrontCarBlindSpotCarRearCarHeadingLeft(
			 * theListOfTheLaneTheTargetedCarIsOn, LaneChangeDirection.LEFT);
			 * 
			 * break; case TargetLane.HEADING_LEFT_ON_LEFT_LANE_LIST:
			 * setTargetLaneFrontCarBlindSpotCarRearCarHeadingLeft(
			 * theListOfTheLaneTheTargetedCarIsOn, LaneChangeDirection.RIGHT); break; case
			 * TargetLane.HEADING_LEFT_IN_LEFTTURNBOX_LIST:
			 * setTargetLaneFrontCarBlindSpotCarRearCarHeadingLeft(
			 * theListOfTheLaneTheTargetedCarIsOn, LaneChangeDirection.LEFT); break; case
			 * TargetLane.HEADING_RIGHT_ON_RIGHT_LANE_LIST:
			 * setTargetLaneFrontCarBlindSpotCarRearCarHeadingRight(
			 * theListOfTheLaneTheTargetedCarIsOn, LaneChangeDirection.LEFT); break; case
			 * TargetLane.HEADING_RIGHT_ON_LEFT_LANE_LIST:
			 * setTargetLaneFrontCarBlindSpotCarRearCarHeadingRight(
			 * theListOfTheLaneTheTargetedCarIsOn, LaneChangeDirection.RIGHT); break;
			 * 
			 * case TargetLane.HEADING_RIGHT_IN_LEFTTURNBOX_LIST:
			 * setTargetLaneFrontCarBlindSpotCarRearCarHeadingRight(
			 * theListOfTheLaneTheTargetedCarIsOn, LaneChangeDirection.LEFT); break; case
			 * TargetLane.HEADING_UP_ON_RIGHT_LANE_LIST:
			 * setTargetLaneFrontCarBlindSpotCarRearCarHeadingUp(
			 * theListOfTheLaneTheTargetedCarIsOn, LaneChangeDirection.LEFT); break; case
			 * TargetLane.HEADING_UP_ON_LEFT_LANE_LIST:
			 * setTargetLaneFrontCarBlindSpotCarRearCarHeadingUp(
			 * theListOfTheLaneTheTargetedCarIsOn, LaneChangeDirection.RIGHT); break; case
			 * TargetLane.HEADING_UP_IN_LEFTTURNBOX_LIST:
			 * setTargetLaneFrontCarBlindSpotCarRearCarHeadingUp(
			 * theListOfTheLaneTheTargetedCarIsOn, LaneChangeDirection.LEFT); break;
			 */
	...
	/*
	 * remember to fix the TargetLane Enum to a a DEFAULTEMPTYLIST and Initialize a
	 * Default OberservableList That's Empty
	 * 
	 * ///step 1: change the function Name by tweaking the "Heading---" Part of the
	 * name
	 * 
	 * ///step 2: double temp_Front_Distance---Can be X or Y may need adjustment to
	 * the name /// double temp_Rear_Distance--- same thing ^ /// that's it ///
	 * ///step 3: targetLane_Nearest_Front_Car_Base_On_DistanceHeading---need
	 * tweaking for "Heading---XorY" Part of the name ///
	 * targetLane_Nearest_Rear_Car_Base_On_DistanceHeading--- same thing ^ ///
	 * ///step 4: isWithinBlindSpotFrontBound_Heading---need tweaking for
	 * "Heading---" Part of the name (Be sure to create the Helper function that it
	 * is) /// isWithinBlindSpotRearBound_Heading---need tweaking for same thing ^
	 * /// ///step 5: (first if statement inside
	 * else)((helper)isFrontCar_Heading---Need direction) /// if statement inside
	 * (universal helper)
	 * isTempFrontDistanceGreaterThanLengthOfTheBlindSpotCarAndLessThanGreatestDistanceOfTheNearestFrontCar
	 * /// (...first argument like step 2 for Front, second argument no change,
	 * third argument like step 3 for the front one) /// inside(this if statement)
	 * like step 3 for (the Front one)-->targetLane_Nearest--blahblah (step 6) ///
	 * /// (Second if statement inside else)((helper)isRearCar_Heading---Need
	 * Direction) /// if statement inside (universal helper)
	 * isTempRearDistanceGreaterThanLengthOfTheBlindSpotCarAndLessThanGreaTestDistanceOfTheNearestRearCar
	 * /// (...first argument like step 2 for Rear, second argument no change, third
	 * argument like step 3 for the Rear one) /// inside(this if statement) like
	 * step 3 for (the Rear one)-->targetLane_Nearest--blahblah (step 6)
	 * 
	 */
	/*
	 * New Universal Helper Functions step 1: getTemp_FrontDistance--XorY //Done
	 * 
	 * step 2: getTemp_RearDistance--XorY //Done
	 */
	public void setTargetLaneCarsDetectionHeadingDown(
			ObservableList<IntersectionSimCar> theListOfTheLaneTheTargetedCarIsOn,
			LaneChangeDirection laneChangeDirection) {// Done
        setPrimaryBlindSpotCarFrontCar(null);
		setPrimaryBlindSpotCar(null);
		setPrimaryBlindSpotCarRearCar(null);
		setFrontBlindSpotCarFrontCar(null);
		setFrontBlindSpotCar(null);
		setFrontBlindSpotCarRearCar(null);
		setFrontBlindSpotCarFrontCar(null);
		setRearBlindSpotCarFrontCar(null);
		setRearBlindSpotCar(null);
		setRearBlindSpotCarRearCar(null);
		setFrontCarOnTargetLaneList(null);
		setRearCarOnTargetLaneList(null);
		String nameOfMethodCalled = "setTargetLaneCarsDetectionHeadingDown(...)";
		String nameOfConditionAccessedFrontCar = "isFrontCar_HeadingDown()";
		String nameOfConditionAccessedRearCar = "isRearCar_HeadingDown()";
		String usedInPlaceOfConditionNameForDebugger = "Outside of For Loop";
		double temp_Front_DistanceY;
		double temp_Rear_DistanceY;
		IntersectionSimCar temp_TargetLaneListFrontCar = null;
		IntersectionSimCar temp_TargetLaneListRearCar = null;
		targetLane_Nearest_Front_Car_Base_On_DistanceHeadingDownY = MAX_Y_DISTANCE_BETWEEN_CARS;
		targetLane_Nearest_Rear_Car_Base_On_DistanceHeadingDownY = MAX_Y_DISTANCE_BETWEEN_CARS;
		for (IntersectionSimCar car : theListOfTheLaneTheTargetedCarIsOn) {
			
			if (this.getTargetLaneList() == TargetLane.EMPTYLIST  || car == null) {
				continue;
			} else {

				if (isPrimaryBlindSpotCar(car)) {
					this.setPrimaryBlindSpotCar(car != null ? (IntersectionSimCar) car : null);
					this.setPrimaryBlindSpotCarFrontCar(
							primaryBlindSpotCar != null && primaryBlindSpotCar.getFrontCar() != null
									? (IntersectionSimCar) primaryBlindSpotCar.getFrontCar()
									: null);
					this.setPrimaryBlindSpotCarRearCar(
							primaryBlindSpotCar != null & primaryBlindSpotCar.getRearCar() != null
									? (IntersectionSimCar) primaryBlindSpotCar.getRearCar()
									: null);
					...
					if (isFrontBlindSpotCar(car)) {
						this.setFrontBlindSpotCar(car != null ? (IntersectionSimCar) car : null);
						this.setFrontBlindSpotCarFrontCar(
								frontBlindSpotCar != null && frontBlindSpotCar.getFrontCar() != null
										? (IntersectionSimCar) frontBlindSpotCar.getFrontCar()
										: null);
						this.setFrontBlindSpotCarRearCar(
								frontBlindSpotCar != null && frontBlindSpotCar.getRearCar() != null
										? (IntersectionSimCar) frontBlindSpotCar.getRearCar()
						
										: null);

					}
					if (isRearBlindSpotCar(car)) {
						this.setRearBlindSpotCar(car != null ? (IntersectionSimCar) car : null);
						this.setRearBlindSpotCarRearCar(
								rearBlindSpotCar != null && rearBlindSpotCar.getRearCar() != null
										? (IntersectionSimCar) rearBlindSpotCar.getRearCar()
										: null);
						this.setRearBlindSpotCarFrontCar(
								rearBlindSpotCar != null && rearBlindSpotCar.getFrontCar() != null
										? (IntersectionSimCar) rearBlindSpotCar.getFrontCar()
										: null);
			...
			if (this.getTargetLaneList() == TargetLane.EMPTYLIST  || car == null) {
				continue;
					...
					if (isFrontCar_HeadingDown(car)) {
						temp_Front_DistanceY = getTemp_FrontDistanceY(laneChangeDirection, car);
						if(isTempFrontDistanceLessThanGreatestDistanceOfTheNearestFrontCar(temp_Front_DistanceY, this.targetLane_Nearest_Front_Car_Base_On_DistanceHeadingDownY)){
							targetLane_Nearest_Front_Car_Base_On_DistanceHeadingDownY = temp_Front_DistanceY;
							temp_TargetLaneListFrontCar = car;
					...
					if (isRearCar_HeadingDown(car)) {
						    ...
						    temp_Rear_DistanceY = getTemp_RearDistanceY(laneChangeDirection, car);
							if(isTempRearDistanceLessThanGreatestDistanceOfTheNearestRearCar(temp_Rear_DistanceY, this.targetLane_Nearest_Rear_Car_Base_On_DistanceHeadingDownY)){
	...
	private void debuggerForTargetLaneCarsDetection(String nameOfMethodCalled, String nameOfConditionAccessed) {
		System.out.println(
				"\n\n--------------------------------------------------------------------------------------------------------------------------------------------------------------"
						
						+ "\n "+ nameOfMethodCalled + " Called --> Condition --> "+ nameOfConditionAccessed + "\n targetLaneListKey: "
						+ this.getTargetLaneListKey() + "\n --->this: " + this.getCarSkin() + " ID: "
						+ this.getCarId() + " ---->                 OnWhichLane: "
						+ this.getOnWhichLaneListKey() + "\n --->this: " + this.getCarSkin() + " ID: "
						+ this.getCarId() + " ---->      frontCarOnTheOtherLane: "
						+ (this.getFrontCarOnTargetLaneList() == null ? null
								: this.getFrontCarOnTargetLaneList().getCarSkin())
						+ " ID: "
						+ (this.getFrontCarOnTargetLaneList() == null ? null
								: this.getFrontCarOnTargetLaneList().getCarId())
						+ "\n --->this: " + this.getCarSkin() + " ID: " + this.getCarId()
						+ " ---->   frontBlindSpotCarFrontCar: "
						+ (this.getFrontBlindSpotCarFrontCar() == null ? null
								: this.getFrontBlindSpotCarFrontCar().getCarSkin())
						+ " ID: "
						+ (this.getFrontBlindSpotCarFrontCar() == null ? null
								: this.getFrontBlindSpotCarFrontCar().getCarId())
						+ "\n --->this: " + this.getCarSkin() + " ID: " + this.getCarId()
						+ " ---->           frontblindSpotCar: "
						+ (this.getFrontBlindSpotCar() == null ? null
								: this.getFrontBlindSpotCar().getCarSkin())
						+ " ID: "
						+ (this.getFrontBlindSpotCar() == null ? null
								: this.getFrontBlindSpotCar().getCarId())
						+ "\n --->this: " + this.getCarSkin() + " ID: " + this.getCarId()
						+ " ---->    frontBlindSpotCarRearCar: "
						+ (this.getFrontBlindSpotCarRearCar() == null ? null
								: this.getFrontBlindSpotCarRearCar().getCarSkin())
						+ " ID: "
						+ (this.getFrontBlindSpotCarRearCar() == null ? null
								: this.getFrontBlindSpotCarRearCar().getCarId())
						+ "\n --->this: " + this.getCarSkin() + " ID: " + this.getCarId()
						+ " ----> primaryblindSpotCarFrontCar: "
						+ (this.getPrimaryBlindSpotCarFrontCar() == null ? null
								: this.getPrimaryBlindSpotCarFrontCar().getCarSkin())
						+ " ID: "
						+ (this.getPrimaryBlindSpotCarFrontCar() == null ? null
								: this.getPrimaryBlindSpotCarFrontCar().getCarId())
						+ "\n --->this: " + this.getCarSkin() + " ID: " + this.getCarId()
						+ " ---->         primaryblindSpotCar: "
						+ (this.getPrimaryBlindSpotCar() == null ? null
								: this.getPrimaryBlindSpotCar().getCarSkin())
						+ " ID: "
						+ (this.getPrimaryBlindSpotCar() == null ? null
								: this.getPrimaryBlindSpotCar().getCarId())
						+ "\n --->this: " + this.getCarSkin() + " ID: " + this.getCarId()
						+ " ---->  primaryblindSpotCarRearCar: "
						+ (this.getPrimaryBlindSpotCarRearCar() == null ? null
								: this.getPrimaryBlindSpotCarRearCar().getCarSkin())
						+ " ID: "
						+ (this.getPrimaryBlindSpotCarRearCar() == null ? null
								: this.getPrimaryBlindSpotCarRearCar().getCarId())
						+ "\n --->this: " + this.getCarSkin() + " ID: " + this.getCarId()
						+ " ---->    rearblindSpotCarFrontCar: "
						+ (this.getRearBlindSpotCarFrontCar() == null ? null
								: this.getRearBlindSpotCarFrontCar().getCarSkin())
						+ " ID: "
						+ (this.getRearBlindSpotCarFrontCar() == null ? null
								: this.getRearBlindSpotCarFrontCar().getCarId())
						+ "\n --->this: " + this.getCarSkin() + " ID: " + this.getCarId()
						+ " ---->            rearblindSpotCar: "
						+ (this.getRearBlindSpotCar() == null ? null
								: this.getRearBlindSpotCar().getCarSkin())
						+ " ID: "
						+ (this.getRearBlindSpotCar() == null ? null
								: this.getRearBlindSpotCar().getCarId())
						+ "\n --->this: " + this.getCarSkin() + " ID: " + this.getCarId()
						+ " ---->     rearblindSpotCarRearCar: "
						+ (this.getRearBlindSpotCarRearCar() == null ? null
								: this.getRearBlindSpotCarRearCar().getCarSkin())
						+ " ID: "
						+ (this.getRearBlindSpotCarRearCar() == null ? null
								: this.getRearBlindSpotCarRearCar().getCarId())
						+ "\n --->this: " + this.getCarSkin() + " ID: " + this.getCarId()
						+ " ---->  rearCarOnTheOtherLane: "
						+ (this.getRearCarOnTargetLaneList() == null ? null
								: this.getRearCarOnTargetLaneList().getCarSkin())
						+ " ID: "
						+ (this.getRearCarOnTargetLaneList() == null ? null
								: this.getRearCarOnTargetLaneList().getCarId())
	...
	//ZOMBIE: "UNIVERSAL HELPERS RIGHT HERE!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!"
			...
			private boolean isTempFrontDistanceLessThanGreatestDistanceOfTheNearestFrontCar(double temp_Front_DistanceXorY, double determineFrontCar_Needs_Heading) {
				return ( temp_Front_DistanceXorY < determineFrontCar_Needs_Heading);
			}
			
			private boolean isTempRearDistanceLessThanGreatestDistanceOfTheNearestRearCar(double temp_Rear_DistanceXorY, double determineRearCar_Needs_Heading) {
	...
	private boolean isFrontCar_HeadingDown(IntersectionSimCar car) { // done step 3
		...
		return ((car != primaryBlindSpotCar 
				 && car != frontBlindSpotCar 
			     && car != rearBlindSpotCar )
				? (this.getFrontLeftCornerPositionY() < car.getRearLeftCornerPositionY())
				: false);// Done step 3
	}

	private boolean isRearCar_HeadingDown(IntersectionSimCar car) { // done step 4
		return ((car != primaryBlindSpotCarRearCar
				 && car != frontBlindSpotCar 
			     && car != rearBlindSpotCar )
				? (this.getRearLeftCornerPositionY() > car.getFrontLeftCornerPositionY())
				: false); // Done step 4
...
//--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
	 ...
	 */
	 ...
	 private double getTemp_FrontDistanceY(LaneChangeDirection laneChangeDirection, IntersectionSimCar car) {
		 return (laneChangeDirection == LaneChangeDirection.RIGHT && car != primaryBlindSpotCar
			     && car != frontBlindSpotCar 
			     && car != rearBlindSpotCar ?
	 ...
	 private double getTemp_RearDistanceY(LaneChangeDirection laneChangeDirection, IntersectionSimCar car) {
		 return (laneChangeDirection == LaneChangeDirection.RIGHT && car != primaryBlindSpotCar
				 && car != frontBlindSpotCar 
			     && car != rearBlindSpotCar ?
	 ...
	 private double getTemp_FrontDistanceX(LaneChangeDirection laneChangeDirection, IntersectionSimCar car) {
		 return (laneChangeDirection == LaneChangeDirection.RIGHT && car != primaryBlindSpotCar
				 && car != primaryBlindSpotCarFrontCar & car != primaryBlindSpotCarRearCar
				 && car != frontBlindSpotCar && car != frontBlindSpotCarFrontCar && car != frontBlindSpotCarRearCar
			     && car != rearBlindSpotCar && car != rearBlindSpotCarFrontCar && car != rearBlindSpotCarRearCar?
				 Math.abs(this.getFrontLeftCornerPositionY() - car.getRearLeftCornerPositionY()):
					 Math.abs(this.getFrontRightCornerPositionY() - car.getRearRightCornerPositionY()));
	 }
	 ...
	 // New Construction for New Universal Helper Functions 1:05AM 2025-05-15
	 //Construction for New Universersal Helper Function Completed 1:33AM 2025-05-15
	 //------------------------------------------------------------------------------------------------------------------
//--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
// new helper function for setTargetLaneCarsDetectionHeadingDown(blah)
	...
	private boolean isPrimaryBlindSpotCar(IntersectionSimCar car) {
		...
		return ((this.getFrontRightCornerPositionY() >= car.getFrontRightCornerPositionY() + 3
				&& this.getRearRightCornerPositionY() <= car.getRearRightCornerPositionY() - 3)
				|| (this.getFrontRightCornerPositionY() <= car.getFrontRightCornerPositionY() + 3
						&& this.getRearRightCornerPositionY() >= car.getRearRightCornerPositionY() - 3));
	}

	private boolean isFrontBlindSpotCar(IntersectionSimCar car) {
		return (car != null
				? ((this.getFrontRightCornerPositionY() > car.getRearLeftCornerPositionY() - 2.9999)
						&& (this.getRearRightCornerPositionY() < car.getRearLeftCornerPositionY() - 2.9999))
				: false);
	}
	...
	private boolean isRearBlindSpotCar(IntersectionSimCar car) {
		...
		return (car != null
				? ((this.getFrontRightCornerPositionY() > car.getFrontLeftCornerPositionY() + 2.9999)
						&& (this.getRearRightCornerPositionY() < car.getFrontLeftCornerPositionY() + 2.9999))
				: false);
	}
...
//--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
	...
	// ------------------------------------------------------------------------------------------------------------------
//*****************************************************************************************************************************************************************************************************************
	// Left or right blinkers signal turn on will borrow from setTargetEnum()
	// Helpers functions to determine the necessity to consider turning the blinkers
	// on
	public void turnOnLeftOrRightBlinkersBasedOnCarIntention() {

		if (isBI_ONLLWANTRLC() || isBI_ONRLWANTRT() || isART_ONLLWANTRLC() || isALT_ONLLWANTRLC()
				|| isAI_ONLLWANTRLC()) {
			rightBlinkersShouldBlinkAtTheRightMoment();
		} else if (isBI_ONRLWANTLLC() || isBI_ONLLWANTLLC() || isBI_ONLTBWANTLT() || isART_ONRLWANTLLC()
				|| isALT_ONRLWANTLLC() || isAI_ONRLWANTLLC()) {
	...
	// *****************************************************************************************************************************************************************************************************************
	/**
	 * kinda have an idea of how to keep track of the car making a right turn or a
	 * left turn the Center point of the left turn circle the car path is tracing is
	 * (TOP_MAKE_LEFT_POINTX + LEFTTURNRADIUS) is the X (TOP_MAKE_LEFT_POINTY) is
	 * the Y so the Coordinate of the Center Point is (X,Y) and you can use the
	 * pythagorean theorem to calculate the point you want the car to pause and wait
	 * for traffic to clear, the point where the Left turning should end and the car
	 * should be going straight, and that allows you to track and know when left
	 * turn has started and the car progress in the left turn SAME THING CAN BE DONE
	 * FOR THE RIGHT (MAKE CHANGES IN THE CODE LATER!!!!) ^^
	 * 
	 * CHATGPT Math.atan2(dy,dx) GODSEND ATANGENT FOR ALL QUARDRANTS
	 * BABY!!!!!!!!!!!! METHOD FROM A BRILLIANT MIND NOTHING TO DO WITH ME!!!!!
	 */
	@Override
	public void setGeneralPlacementOfCarBasedOnGeneralLocation() {

		double dist_from_LeftTurn_Point = Math.sqrt(Math.pow((TOP_MAKE_LEFT_POINTX - this.getPositionX()), 2)
				+ Math.pow((TOP_MAKE_LEFT_POINTY - this.getPositionY()), 2));// tweak here
		double dist_from_RightTurn_Point = Math.sqrt(Math.pow((TOP_MAKE_RIGHT_POINTX - this.getPositionX()), 2)
				+ Math.pow((TOP_MAKE_RIGHT_POINTY - this.getPositionY()), 2));
		...
		} else if (isOnOriginalLeftLane() && isBlockingLeftTraffic()
				|| isOnOriginalRightLane() && isBlockingLeftTraffic()) {// tweak here
			this.setGeneralPlacementOfCar(GeneralPlacementOfCar.IN_INTERSECTIONBLOCKLT);
		} else if (isOnOriginalLeftLane() && isBlockingRightTraffic()
				|| isOnOriginalRightLane() && isBlockingRightTraffic()) {// tweak here
	...
	private boolean isGoingStraightBeforeIntersection() {
		return this.getPositionY() < TOP_MAKE_RIGHT_POINTY && this.getRotationAngle() == HEADING_DOWN_CARDINAL_ANGLE;// tweak
																														// here
	}

	private boolean isGoingStraightAfterIntersection() {
		return this.getPositionY() > BOTTOM_MAKE_RIGHT_POINTY && this.getRotationAngle() == HEADING_DOWN_CARDINAL_ANGLE;// tweak
																														// here
																														// need
																														// Fs
																														// for
	...
	private boolean isBlockingOnComingTraffic() {
		return this.getPositionX() > TOP_MAKE_LEFT_POINTX && this.getPositionX() < BOTTOM_MAKE_RIGHT_POINTX;// tweak
																											// here need
																											// blinkers
																											// for it to
																											// work
	...
	private boolean isInsideLeftTurnBox() {
		return Math.abs(HEADING_DOWN_CENTER_OF_LTB_LANE_X - this.getPositionX()) < TOLERANCE
				&& this.getRotationAngle() == HEADING_DOWN_CARDINAL_ANGLE;
	}

	private boolean isBlockingLeftTraffic() {
		return this.getPositionY() >= TOP_MAKE_RIGHT_POINTY + 2.5 && this.getPositionY() <= TOP_MAKE_LEFT_POINTY + 5.5;// tweak
																														// here
																														// need
																														// blinkers
																														// for
	...
	private boolean isBlockingRightTraffic() {
		return this.getPositionY() > TOP_MAKE_LEFT_POINTY + 5.5
				&& this.getPositionY() <= BOTTOM_MAKE_RIGHT_POINTY - 2.5;// tweak here need blinkers for it to work
	}
	...
	private boolean isOnRightLaneAfterRightTurn() {// tweak
		return Math.abs(HEADING_LEFT_CENTER_OF_R_LANE_Y - this.getPositionY()) < TOLERANCE
				&& this.getRotationAngle() == HEADING_LEFT_CARDINAL_ANGLE;
	}

	private boolean isOnLeftLaneAfterRightTurn() {// tweak
		return Math.abs(HEADING_LEFT_CENTER_OF_L_LANE_Y - this.getPositionY()) < TOLERANCE
				&& this.getRotationAngle() == HEADING_LEFT_CARDINAL_ANGLE;
	}

	private boolean isOnRightLaneAfterLeftTurn() {// tweak
		return Math.abs(HEADING_RIGHT_CENTER_OF_R_LANE_Y - this.getPositionY()) < TOLERANCE
				&& this.getRotationAngle() == HEADING_RIGHT_CARDINAL_ANGLE;
	}

	private boolean isOnLeftLaneAfterLeftTurn() {// tweak
		return Math.abs(HEADING_RIGHT_CENTER_OF_L_LANE_Y - this.getPositionY()) < TOLERANCE
				&& this.getRotationAngle() == HEADING_RIGHT_CARDINAL_ANGLE;
	}
	...
	public HeadedDownOrigCar(Pane root, LaneManagement laneManagement, double spawnPositionX, double spawnPositionY) {
		...
		front_R_BlinkerImage_P = new Image(getClass()
				.getResource("/Signal-lights-for-intersection-programOriginal/New-folder/Turn_Signals_002LB.png")
				.toExternalForm());
		front_L_BlinkerImage_P = new Image(getClass()
				.getResource("/Signal-lights-for-intersection-programOriginal/New-folder/Turn_Signals_002LB.png")
				.toExternalForm());
		rear_R_BlinkerImage_P = new Image(getClass()
				.getResource("/Signal-lights-for-intersection-programOriginal/New-folder/Turn_Signals_002LB.png")
				.toExternalForm());
		rear_L_BlinkerImage_P = new Image(getClass()
				.getResource("/Signal-lights-for-intersection-programOriginal/New-folder/Turn_Signals_002LB.png")
				.toExternalForm());
		front_R_BlinkerImage2_P = new Image(getClass()
				.getResource("/Signal-lights-for-intersection-programOriginal/New-folder/Turn_Signals_002.png")
				.toExternalForm());
		front_L_BlinkerImage2_P = new Image(getClass()
				.getResource("/Signal-lights-for-intersection-programOriginal/New-folder/Turn_Signals_002.png")
				.toExternalForm());
		rear_R_BlinkerImage2_P = new Image(getClass()
				.getResource("/Signal-lights-for-intersection-programOriginal/New-folder/Turn_Signals_002.png")
				.toExternalForm());
		rear_L_BlinkerImage2_P = new Image(getClass()
				.getResource("/Signal-lights-for-intersection-programOriginal/New-folder/Turn_Signals_002.png")
				.toExternalForm());
		front_R_BlinkerImage3_P = new Image(getClass()
				.getResource(
						"/Signal-lights-for-intersection-programOriginal/New-folder/Turn_Signals_002_brightest.png")
				.toExternalForm());
		front_L_BlinkerImage3_P = new Image(getClass()
				.getResource(
						"/Signal-lights-for-intersection-programOriginal/New-folder/Turn_Signals_002_brightest.png")
				.toExternalForm());
		rear_R_BlinkerImage3_P = new Image(getClass()
				.getResource(
						"/Signal-lights-for-intersection-programOriginal/New-folder/Turn_Signals_002_brightest.png")
				.toExternalForm());
		rear_L_BlinkerImage3_P = new Image(getClass()
				.getResource(
						"/Signal-lights-for-intersection-programOriginal/New-folder/Turn_Signals_002_brightest.png")
				.toExternalForm());
		...
		if (randomInt >= 0 && randomInt < 20) {
			...
			carImage_brake_P = new Image(
					getClass().getResource("/car-sprite2/blue-race-car-brake.png").toExternalForm());
			...
			carImage_brake_P = new Image(
					getClass().getResource("/car-sprite2/finished-blue-byd-brake.png").toExternalForm());
			...
			carImage_brake_P = new Image(
					getClass().getResource("/car-sprite2/red-corrolla-brake.png").toExternalForm());
			...
			carImage_brake_P = new Image(
					getClass().getResource("/car-sprite2/finished-grey2-byd-brake.png").toExternalForm());
			...
			carImage_brake_P = new Image(
					getClass().getResource("/car-sprite2/finished-grey-byd-brake.png").toExternalForm());
			...
			carImage_brake_P = new Image(
					getClass().getResource("/car-sprite2/finished-teal-byd2-brake.png").toExternalForm());
			...
			carImage_brake_P = new Image(
					getClass().getResource("/car-sprite2/Yellow-car-sprite3-brake.png").toExternalForm());
			...
			carImage_brake_P = new Image(
					getClass().getResource("/car-sprite2/Red-car-sprite-brake.png").toExternalForm());
			...
			carImage_brake_P = new Image(
					getClass().getResource("/car-sprite2/Green-car-sprite-brake.png").toExternalForm());
		...
		positionBlinkersImageView(imageViewsAngle, this.front_R_BlinkerImageView_P, this.front_L_BlinkerImageView_P,
				this.rear_R_BlinkerImageView_P, this.rear_L_BlinkerImageView_P);
		positionBlinkersImageView(imageViewsAngle, this.front_R_BlinkerImageView2_P, this.front_L_BlinkerImageView2_P,
				this.rear_R_BlinkerImageView2_P, this.rear_L_BlinkerImageView2_P);
		positionBlinkersImageView(imageViewsAngle, this.front_R_BlinkerImageView3_P, this.front_L_BlinkerImageView3_P,
				this.rear_R_BlinkerImageView3_P, this.rear_L_BlinkerImageView3_P);
		...
		// Rotate around the center of the ImageView, now aligned with the desired fixed
		// coordinate
		...
		Translate backToPosition2 = new Translate(calculatedLayoutPointForOffCenterFixedPointX,
				calculatedLayoutPointForOffCenterFixedPointY);

		this.carImageView_P.getTransforms().clear();// you need to clear it so the angle return back to its original
													// position because the previous angle matters
		this.carImageView_P.getTransforms().addAll(translateToPivot2, rotate3, backToPosition2);

		this.carImageView_brake_P.getTransforms().clear();// you need to clear it so the angle return back to its
															// original position because the previous angle matters
	 ...
	 */
	// set the object properties: rise, run, velocityWanted, riseOverRun,
	// runOverRise, distanceTraveledYaxis, distanceTraveledXaxis
	...
	public void positionManipulationFormula(double startLaneChangeTurnX, double startLaneChangeTurnY,
		...
		// distanceTraveledXaxis = 60 * Math.sqrt(Math.pow(velocityWanted,2)/
		// (Math.pow(riseOverRun,2) + 1));
		// distanceTraveledYaxis = 60 * Math.sqrt(Math.pow(velocityWanted,2)/
		// (Math.pow(runOverRise,2) + 1));
		...
		double remainingDistanceXab = Math.abs(getPositionX() - straightEndingX); // absolute value of the remaining
																					// Distance in the x-axis at the end
																					// of phase 2 in a lane change
																					// operation
		double remainingDistanceYab = Math.abs(getPositionY() - straightEndingY); // absolute value of the remaining
																					// Distance in the y-axis at the end
																					// of phase 2 in a lane change
																					// operation
		// double remainingDistanceHDFCarY = Math.abs(270 -
		// this.getFrontRightCornerPositionY());
		/*
		 * System.out.
		 * println("this.beforeInt_RightLaneChangePhase == LaneChangePhase.PHASE_2: " +
		 * (this.beforeInt_RightLaneChangePhase == LaneChangePhase.PHASE_2) + "\n"
		 * +"remainingDistanceXab = " + remainingDistanceXab +
		 * " , distanceTraveledXaxis =" + Math.abs(distanceTraveledXaxis) + "\n"
		 * +"remainingDistanceYab = " + remainingDistanceYab +
		 * " , distanceTraveledYaxis =" + Math.abs(distanceTraveledYaxis) + "\n");
		 */

		if (getCarIntention() == CarIntention.BI_ONLTBWANTLT
				&& (HEADINGDOWNLEFTTURNY - getPositionY()) < getDistanceTraveledYaxis()) {
		...
		} else if (getCarIntention() == CarIntention.BI_ONLLWANTRLC
				&& remainingDistanceY_Test < getDistanceTraveledYaxis()) {
			setDistanceTraveledYaxis(remainingDistanceY_Test);

		} else if (getCarIntention() == CarIntention.ALT_ONLLWANTRLC
				&& remainingDistanceX_Test < getDistanceTraveledXaxis()) {
			setDistanceTraveledXaxis(remainingDistanceX_Test);

		} else if (getAfterLT_RightLaneChangePhase() == LaneChangePhase.PHASE_2
				&& remainingDistanceXab < Math.abs(getDistanceTraveledXaxis())
				&& remainingDistanceYab < Math.abs(getDistanceTraveledYaxis())) {
			setDistanceTraveledYaxis(remainingDistanceYab);
			setDistanceTraveledXaxis(remainingDistanceXab);
		} else if (getBeforeInt_RightLaneChangePhase() == LaneChangePhase.PHASE_2
				&& remainingDistanceXab < Math.abs(getDistanceTraveledXaxis())
				&& remainingDistanceYab < Math.abs(getDistanceTraveledYaxis())) {
			// System.out.println("positionManipulationFormula(****)
			// -->this.beforeInt_RightLanePhase == LaneChangePhase.Phase2--->accessed");
			setDistanceTraveledYaxis(Math.sin(normalizedRotationAngle) / Math.abs(Math.sin(normalizedRotationAngle))
					* remainingDistanceYab);
			setDistanceTraveledXaxis(Math.cos(normalizedRotationAngle) / Math.abs(Math.cos(normalizedRotationAngle))
...
//****************************************************************************************************************************************************************************************************************
	...
	public void setCarActionState() {
		...
		if (this.shouldDo_AfterLeftTurn_RightLaneChange()) {
			...
			// System.out.println("CircleRpCar.java line: 796 carID: "+this+ " CarIntention:
			// " + carIntention);

			carAction = CarAction.ALT_RIGHTLANECHANGE;
			// System.out.println("CircleRpCar.java line 801 accessed ---car ID: " + this +
			// " | Phase: " + afterLT_RightLaneChangePhase + " | PositionX: " + positionX);

		} else if (this.shouldDo_BeforeInt_RightLaneChange()) {
			// System.out.println("setCarActionState() -->
			// this.shouldDo_BeforeInt_RightLaneChange()");
		...
		// this.bI_CancelRightLaneChangePhaseState();//Wrong Reason Get Rid for now
		// because of shouldCancelRightLaneChange helper function
	...
	/**
	 * Decision setting the state in right lane change mode after a left turn after
	 * conditions are met condition: if car has intention for right lane change and
	 * if car reached turning point position
	 * 
	...
	/**
	 * Decision setting the state in right lane change mode before Intersection
	 * after conditions are met condition: if car has intention for right lane
	 * change and if car reached turning point position
	 * 
	 * @return boolean True/false
	 */
	private boolean shouldDo_BeforeInt_RightLaneChange() {
		// System.out.println("shouldDo_BeforeInt_RightLaneChange()--->carIntention:" +
		// carIntention);
		return carIntention == CarIntention.BI_ONLLWANTRLC && this.positionY == this.yCoordinate;
	}

	/**
	 * if carAction's state is not set to (after left turn right lane change) and if
	 * the car is not facing a cardinal angle
	 * 
	 * @return boolean True/false
	 */
	private boolean shouldGoStraight() {

		return carAction != CarAction.ALT_RIGHTLANECHANGE && carAction != CarAction.ALT_CANCELRIGHTLANECHANGE
				&& (rotationAngle == 180 || rotationAngle == 270 || rotationAngle == 90 || rotationAngle == 0);
	}

	/**
	 * need to think about it **if the car is on the leftTurnBox lane then it has to
	 * turn left need to WORK ON THIS CONDITION
	 * 
	 * @return
	 */
	 ...
	 * 
	 * @param carImageView       --ImageView contained the skin associated with the
	 *                           CircleRpCar Object
	 * @param carImageView_brake --ImageView contained the skin (Brake Light On)
	 *                           associated with the CircleRpCar Object
	 */
	public void actionBaseOnCarState(ImageView carImageView, ImageView carImageView_Brake) {// May Need tweaking here
		...
		case CarAction.BI_RIGHTLANECHANGE:
			// System.out.println("actionBaseOnCarState(ImageView, ImageView) --> case
			// carAction.BI_RIGHTLANECHANGE accessed");
	...
	public void carOperation() {
		...
		positionManipulationFormula(this.getxCoordinate(), this.getyCoordinate(), getStraightEndingX(),
				getStraightEndingY());
		...
		this.needToDoThisEverytimeSetTargetFrontCarBlindSpotCarRearCar();
		// this.laneChangePointXSelection();// need to come up with lane change decision
		// logic
		 ...
		 * "\n --->this: " + this.getCarSkin() + "---->        OnWhichLane: " +
		 * this.getOnWhichLaneListKey() + "\n --->this: " + this.getCarSkin() +
		 * "getObservablelistCarisOn(): " +
		 * this.laneManagement.getHashMap_For_Observablelist().get(getOnWhichLaneListKey
		 * ()) + "\n --->this: " + this.getCarSkin() + "---->           frontCar: " +
		 * (this.getFrontCar() == null ? null : this.getFrontCar().getCarSkin()) +
		 * 
		 * "\n --->this: " + this.getCarSkin() + "---->            rearCar: " +
		 * (this.getRearCar() == null ? null : this.getRearCar().getCarSkin()) );
		 */
	...
	/**
	 * needed for initialization and continued calculation to keep blinkers in their
	 * proper place along with the car's movement
	 * 
	 * @param angle_Difference_Front_Blinkers --> the fixed angle difference between
	 *                                        the rotation angle and the angle a
	 *                                        front blinker needs to be at
	 * @param angle_Difference_Back_Blinkers  --> the fixed angle difference between
	 *                                        the rotation angle and the angle a
	 *                                        rear blinker needs to be at
	 */
	public void calculateAndSetAllBlinkersAngle(double angle_Difference_Front_Blinkers,
			double angle_Difference_Back_Blinkers) {
	...
	/**
	 * need for initialization and continued calculation to keep blinkers in their
	 * proper place along with the car's movement
	 * 
	 * @param length_CarPosToFrontBlinker --the fixed length between the primary
	 *                                    reference point such as
	 *                                    (positionX,positionY) and the front
	 *                                    Blinker Position (blahblah....X,
	 *                                    blahblah....Y)
	 * @param length_CarPosToRearBlinker  --the fixed length between the primary
	 *                                    reference point such as
	 *                                    (positionX,positionY) and the rear Blinker
	 *                                    Position (blahblah....X, blahblah....Y)
	 */
	...
	// -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
	public void calculateAndSetAllCornersAngle(double angle_Difference_Front_Corners,
			double angle_Difference_Back_Corners) {
	...
	/*
	 * public ObservableList<Car>
	 * getTargetObservaleListOfCars(CircleRpCar.OnWhichLane TargetLane){ return
	 * this.getHashMap_For_Observablelist().get(TargetLane); }
	 */
	/**
	 ...
	 *
	 * Note: - getObservableListCarIsOn() depends on `onWhichLaneListKey`, which is
	 * not updated until a lane change or turn (left/right) is completed and the car
	 ...
	 *
	 * The onWhichLaneListKey resets the nearest car distances based on current car
	 * skin size and heading direction.
	 */
	...
	// ---------------------------------------------------------------------------------------------------------------------
	/**
	 * The reason there is a switch statement in there is because it need to align
	 * this.onWhichLaneListKey with the method to set the front car and the rear car
	 * depending on the direction such as Heading Up...or Left etc
	 * 
	 * @param theListOfTheLaneTheCarIsOn --> The Only Argument That that should be
	 *                                   passed into this parameter is
	 *                                   "this.hashMap_For_Observablelist.get(onWhichLaneListKey)"
	 *                                   because the value will align with the
	 *                                   this.onWhichLaneListKey which is part of
	 *                                   the OnWhichLane enum
	 */
	...
	public void setFrontCarRearCarHeadingLeft(ObservableList<IntersectionSimCar> theListOfTheLaneTheCarIsOn) {
		...
		for (IntersectionSimCar car : theListOfTheLaneTheCarIsOn) {
			if (this == car) {
				...
				if (((IntersectionSimCar) this).getFrontLeftCornerPositionX() > ((IntersectionSimCar) car)
						.getRearLeftCornerPositionX()) {

					temp_Front_DistanceX = Math.abs(this.getFrontLeftCornerPositionX()
							- ((IntersectionSimCar) car).getRearLeftCornerPositionX());
				...
				if (((IntersectionSimCar) this).getRearLeftCornerPositionX() < car.getFrontLeftCornerPositionX()) {
					temp_Rear_DistanceX = Math.abs(this.getRearLeftCornerPositionX()
							- ((IntersectionSimCar) car).getFrontLeftCornerPositionX());
	...
	public void setFrontCarRearCarHeadingRight(ObservableList<IntersectionSimCar> theListOfTheLaneTheCarIsOn) {
		...
		for (IntersectionSimCar car : theListOfTheLaneTheCarIsOn) {
			if (this == car) {
				...
				if (((IntersectionSimCar) this).getFrontRightCornerPositionX() < ((IntersectionSimCar) car)
						.getRearRightCornerPositionX()) {
					temp_Front_DistanceX = Math.abs(this.getFrontRightCornerPositionX()
							- ((IntersectionSimCar) car).getRearRightCornerPositionX());
				...
				if (((IntersectionSimCar) this).getRearLeftCornerPositionX() > ((IntersectionSimCar) car)
						.getFrontLeftCornerPositionX()) {
					temp_Rear_DistanceX = Math.abs(this.getRearLeftCornerPositionX()
							- ((IntersectionSimCar) car).getFrontLeftCornerPositionX());
					if (temp_Rear_DistanceX < nearest_Rear_Car_Base_On_DistanceHeadingLeftX) {
		...
		for (IntersectionSimCar car : theListOfTheLaneTheCarIsOn) {
			if (this == car) {
				...
				if (this.getFrontRightCornerPositionY() > car.getRearRightCornerPositionY()) {
					temp_Front_DistanceY = Math
							.abs(this.getFrontRightCornerPositionY() - car.getRearRightCornerPositionY());
				...
				if (this.getRearRightCornerPositionY() < car.getFrontRightCornerPositionY()) {
					temp_Rear_DistanceY = Math
							.abs(this.getRearRightCornerPositionY() - car.getFrontRightCornerPositionY());
	...
	public void setFrontCarRearCarHeadingDown(ObservableList<IntersectionSimCar> theListOfTheLaneTheCarIsOn) {
		...
		double temp_Rear_DistanceY;
		// System.out.println("\n --->this: " + this.getCarSkin() +
		// "getObservablelistCarisOn(): " + this.getObservableListCarIsOn());
		for (IntersectionSimCar car : theListOfTheLaneTheCarIsOn) {
			if (this == car) {
				continue;
			} else {
				if (this.getFrontLeftCornerPositionY() < car.getRearLeftCornerPositionY()) {
					// System.out.println("setFrontCarRearCarHeadingDown: setFrontCar(...) if
					// statement true");
					temp_Front_DistanceY = Math
							.abs(this.getFrontLeftCornerPositionY() - car.getRearLeftCornerPositionY());
				...
				if (this.getRearLeftCornerPositionY() > car.getFrontLeftCornerPositionY()) {
					// System.out.println("setFrontCarRearCarHeadingDown: setRearCar(...) if
					// statement true");
					temp_Rear_DistanceY = Math.abs(((IntersectionSimCar) this).getRearLeftCornerPositionY()
							- car.getFrontLeftCornerPositionY());
...
//-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
	public void positionBlinkersImageView(double imageViewAngle, ImageView front_R_BlinkerImageView,
			ImageView front_L_BlinkerImageView, ImageView rear_R_BlinkerImageView, ImageView rear_L_BlinkerImageView) {
		...
		double calculatedLayoutPointForCenterFrontRBlinkerX = frontRBlinkerX
				- front_R_BlinkerImageView.getFitWidth() / 2;
		double calculatedLayoutPointForCenterFrontRBlinkerY = frontRBlinkerY
				- front_R_BlinkerImageView.getFitHeight() / 2;
		double calculatedLayoutPointForCenterFrontLBlinkerX = frontLBlinkerX
				- front_L_BlinkerImageView.getFitWidth() / 2;
		double calculatedLayoutPointForCenterFrontLBlinkerY = frontLBlinkerY
				- front_L_BlinkerImageView.getFitHeight() / 2;
		...
		// Rotate around the center of the ImageView, and now align with the desired
		// blinker coordinate
		...
		// Translate the ImageView back to its original position
		Translate backToPositionFR = new Translate(calculatedLayoutPointForCenterFrontRBlinkerX,
				calculatedLayoutPointForCenterFrontRBlinkerY);
		Translate backToPositionFL = new Translate(calculatedLayoutPointForCenterFrontLBlinkerX,
				calculatedLayoutPointForCenterFrontLBlinkerY);
		Translate backToPositionRR = new Translate(calculatedLayoutPointForCenterRearRBlinkerX,
				calculatedLayoutPointForCenterRearRBlinkerY);
		Translate backToPositionRL = new Translate(calculatedLayoutPointForCenterRearLBlinkerX,
				calculatedLayoutPointForCenterRearLBlinkerY);

		front_R_BlinkerImageView.getTransforms().clear();// you need to clear it so the angle return back to its
															// original position because the previous angle matters
...
//---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
	/**
	 * This is supposed to be the brain function for now the selection of the lane
	 * Change X-Coordinate is randomized
	 */
	public void laneChangePointYSelection() {
		Random random = new Random();

		if (beforeInt_RightLaneChangePhase == LaneChangePhase.COMPLETED && carIntention == CarIntention.BI_ONLLWANTRLC
				&& !yCoordinateSelected
				|| beforeInt_RightLaneChangePhase == LaneChangePhase.NONE && carIntention == CarIntention.BI_ONLLWANTRLC
						&& !yCoordinateSelected) {// tweak here
	...
	public void bI_RightLaneChangePhaseIntention() {
		if (!lane_Change_Probabilities_accessed && this.generalPlacementOfCar == GeneralPlacementOfCar.BI_ONLEFTLANE
			...
			// System.out.println("CircleRpCar.java line: 872 accessed,
			// isXCoordinateSelected: " + this.isxCoordinateSelected());
			laneChangeProbabilities = LaneChangeProbabilities.HIGH;
			laneChangeProbabilities.execute(this);
			// System.out.println("CircleRpCar.java line: 851 carID: "+this+ " CarIntention:
			// " + carIntention);
	...
	public void bI_RightLaneChangePhaseState() {// tweak here
		// System.out.println("bI_RightLaneChangePhaseState()-->accessed,
		// !laneChangePhase1-Initiated = " + laneChangePhase1_Initiated);
		if (shouldBeginBI_RLC_Phase1()) {// tweak here

			beforeInt_RightLaneChangePhase = LaneChangePhase.PHASE_1;// tweak here
			// System.out.println("CircleRpCar.java line 893 accessed ---car ID: " + this +
			// " |Before intersection Phase: " + beforeInt_RightLaneChangePhase + " |
			// PositionX: " + positionX);
			...
			rightBlinkersToggle = RightBlinkersToggle.OFF;
			carAction = CarAction.GOSTRAIGHT;
			// System.out.println("bI_RightLaneChangePhaseState: Lane Change Completed:
	...
	/**
	 * If carAction says (ALT_RIGHTLANECHANGE)--meaning after Left Turn (The car is
	 * on the left lane and It wants to do a right lane change).
	 * laneChangePhase1_initiated has to be false because once phase1 is initiated
	 * it will not initiate phase 1 again until after the lane change is completed.
	 * It is a boolean function for if it should begin phase 1 of the After Left
	 * Turn on left lane right lane change phases
	 * 
	...
	/**
	 * After phase 1 is initiated the car will start turning until it reaches the
	 * optimum angle (right Lane Change ExitAngle A boolean function for if it
	 * should begin phase 2 of the After Left Turn on left lane right lane change
	 * phases
	 * 
	 * @return true or false
	 */
	private boolean shouldBeginBI_RLC_Phase2() {// tweak here
		return this.getRotationAngle() == HeadedDownOrigCar.R_BIRIGHT_LANE_CHANGEEXITANGLE
				&& beforeInt_RightLaneChangePhase == LaneChangePhase.PHASE_1;// tweak here
	}

	/**
	 * A boolean function for if it should begin phase 3 of the After Left Turn on
	 * left lane right lane change phases
	 * 
	 * @return true or false
	 */
	private boolean shouldBeginBI_RLC_Phase3() {// tweak here
		return this.getPositionX() == this.getStraightEndingX() && this.getPositionY() == this.getStraightEndingY()
				&& beforeInt_RightLaneChangePhase == LaneChangePhase.PHASE_2;// tweak here
	}

	// The function Below is the condition for canceling (after left turn) right
	// lane change
	/**
	 * &&&--means need to work on it For this now this function will always = true
	 * when rotationAngle is equal to the R_ALTRIGHT_LANECHANGEEXITANGLE
	 * 
	 * @return true or false
	 */
	...
	/**
	 * A boolean function for if it should begin phase named completed of the After
	 * Left Turn on left lane right lane change phases
	 * 
	 * @return true or false
	 */
	private boolean shouldBeginBI_RLC_PhaseCompleted() {// tweak here
		return this.getRotationAngle() == this.L_BIRIGHT_LANE_CHANGEEXITANGLE
				&& beforeInt_RightLaneChangePhase == LaneChangePhase.PHASE_3;// tweak here
	}
	...
	private void phase2CalculationBI_RLC() {// tweak here
		...
		// System.out.println("angleWithRespectToTheRightTurnCircle = " +
		// this.getAngleWithRespectToTheRightTurnCircle());
		...
		this.setH(startingX - Math.cos(temp_AngleWithRespectToRightTurnCircle) * laneChangeRadius);// h is the x
																									// coordinate of the
																									// center of the
																									// right turn
																									// circle//tweak
																									// here
		this.setK(startingY - Math.sin(temp_AngleWithRespectToRightTurnCircle) * laneChangeRadius);// k is the y
																									// coordinate of the
																									// center of the
																									// right turn
																									// circle//tweak
	...
	public void bI_RightLaneChange() {// tweak here
		...
		case LaneChangePhase.PHASE_2:
			// System.out.println("bI_RightLaneChange() :" +
			// this.getR_BiCancelRight_LaneChangeExitAngle());
			// bI_CancelRightLaneChangePhaseState(); ///DONE AWAY WITH CANCEL LANE CHANGE
			// FOR NOW
			...
			this.positionManipulationFormula(this.getxCoordinate(), this.getyCoordinate(), temp_StraightEndingX,
					temp_StraightEndingY);
			this.gasGoStraight();
			break;
		 ...
		 * //System.out.println("BI_RightLaneChange() --> phase 1: accessed");
		 * 
		 * this.rightTurn(CircleRpCar.getrBiRightLaneChangeExitAngle(), 19.5); }else
		 * if(beforeInt_RightLaneChangePhase == LaneChangePhase.PHASE_2) {//tweak here
		 * 
		 * 
		 * //System.out.println("bI_RightLaneChangePhaseState() :" +
		 * this.getR_BiCancelRight_LaneChangeExitAngle());
		 * bI_CancelRightLaneChangePhaseState();
		 * 
		 * //System.out.println("bI_RightLaneChang--> Phase2"); double
		 * temp_StraightEndingX = this.getStraightEndingX(); double temp_StraightEndingY
		 * = this.getStraightEndingY();
		 * this.positionManipulationFormula(this.getxCoordinate(),this.getyCoordinate(),
		 * temp_StraightEndingX, temp_StraightEndingY); this.gasGoStraight(); }else if
		 * (beforeInt_RightLaneChangePhase == LaneChangePhase.PHASE_3) {//tweak here
		 * 
		 * this.leftTurn(CircleRpCar.getlBiRightLaneChangeExitAngle(), 19.5);//tweak
		 * here ///System.out.
		 * println("BI_RightLaneChange()--> phase 3: accessed, positionX: " +
		 * this.positionX + " " );
		 * 
		 * }
		 */
	...
	public void bI_CancelRightLaneChangePhaseState() {
		if (shouldBICancelRightLaneChange()) {
			// System.out.println("bI_CancelRightLaneChangePhaseState : accessed and
			// beforeInt_RightLaneChangePhase set to CancelLaneChange Phase_1");

			beforeInt_CancelRightLaneChangePhase = CancelLaneChangePhase.PHASE_1;
			// System.out.println("Car ID: " + this + ",
			// beforeInt_CancelRightLaneChangePhase = " +
			// this.beforeInt_CancelRightLaneChangePhase);
		...
		} else if (rotationAngle == this.getR_BiCancelRight_LaneChangeExitAngle()
				&& beforeInt_CancelRightLaneChangePhase == CancelLaneChangePhase.PHASE_1) {
			// System.out.println("bI_CancelRightLaneChangePhaseState : accessed and
			// beforeInt_RightLaneChangePhase set to CancelLaneChange Phase_2");
		...
		// beforeInt_CancelRightLaneChangePhase = " +
		// this.beforeInt_CancelRightLaneChangePhase);
		// System.out.println("bI_CancelRightLaneChangePhaseState() accessed Car ID: "+
	...
	public void bI_CancelRightLaneChange() {
		if (beforeInt_CancelRightLaneChangePhase == CancelLaneChangePhase.PHASE_1) {// tweak here
			// System.out.print("BI_CancelRightLaneChange()
			// accessed---->r_BiCancelRight_LaneChangeExitAngle :" +
			// this.getR_BiCancelRight_LaneChangeExitAngle());
		...
		} else if (beforeInt_CancelRightLaneChangePhase == CancelLaneChangePhase.COMPLETED && rotationAngle == 270) {// tweak
																														// here
